...
// 问题模块
exports.Problem = mongolass.model('Problem', {
  author: { type: Mongolass.Types.ObjectId },
  title: { type: 'string' },
  content: { type: 'string'},
...
exports.Problem.index({ author: 1, _id: -1 }).exec()

// 回答功能模块
exports.Answer = mongolass.model('Answer', {
  author: { type: Mongolass.Types.ObjectId },
  content: { type: 'string' },
  problemId: { type: Mongolass.Types.ObjectId },
  title: { type: 'string' }
});
exports.Answer.index({ problemId: 1, _id: 1 }).exec();// 通过问题 id 获取该问题下所有回答，按回答创建时间升序
exports.Answer.index({ author: 1, _id: 1 }).exec();// 通过用户 id 和回答 id 删除一个回答